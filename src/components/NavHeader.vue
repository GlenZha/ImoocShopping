...
<script>
  ...
  import {mapState} from 'vuex'
    export default {
      ...
      computed:{
        ...mapState(['nickName','cartCount']),//等同于下面的两个函数
          // nickName(){
      ...
      methods:{
        checkLogin(){
          ...
          login(){
            if(!this.userName||!this.userPwd){//进行校验，如果有一个为空就显示错误信息
              this.errorTip=true;
              return;
            }
        ...
        logOut(){
          axios.post("/users/logout").then((response)=>{
            ...
            if(res.status=='0'){
              ...
              this.$store.commit("updateCartCount",0-this.$store.state.cartCount);
            }
          })
        },
        //获取购物车商品总数量
        getCartCount(){
          axios.get("/users/getCartCount").then(ress=>{
            var res = ress.data;
