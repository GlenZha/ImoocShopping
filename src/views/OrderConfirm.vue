...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      //创建订单
      payMent(){
        var addressId=this.$route.query.addressId;//获取地址的ID
        axios.post("/users/payMent",{
          addressId:addressId,//传入地址ID和订单总金额
          orderTotal:this.orderTotal
        }).then((response)=>{
          let res=response.data;
          if(res.status=="0"){//如果正确
            this.$router.push({//跳转到orderSuccess界面并传一个订单ID
